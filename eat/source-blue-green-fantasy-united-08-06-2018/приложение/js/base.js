...
jQuery(".allblock").on('click', '.otherlogin_button_new2_22', function(){
    ...
    urlforkupactivation = 'http://s.geekks.com/social/posttovk.php?llbappkey=1%26repostparams='+repostparams;
    ...
    jQuery.getJSON(url+'/get-achivs-list', {token_key:G_token}, function( q ){
        ...
        if ( q.data.achivs.length != 0 ){
            ...
            for (var i = 0; i < q.data.achivs.length; i++) {
                ...
                //<div class="ach_list_item_body">'+q.data.achivs[i].achivs_body+'</div>
                jQuery('#list_kup_dost_kont').prepend('<div class="ach_list_item" style="background-image:url('+url+q.data.achivs[i].photo+');"><div class="ach_list_item_title">'+q.data.achivs[i].title+'</div><div class="ach_list_item_repost" data-params="'+achparams+'">Поделиться</div></div>');
...
jQuery(".allblock").on('click', '#reposttosoc_vk', function(){
  ...
  if(set_post){
    ...
    cordova.InAppBrowser.open( 'https://oauth.vk.com/authorize?client_id=6385170&display=mobile&redirect_uri='+urlforkupactivation+'&scope=photos,wall&response_type=code', '_system', 'location=no');
...
jQuery(".allblock").on('click', '#reposttosoc_fb', function(){
  ...
  if(set_post){
     ...
     cordova.InAppBrowser.open( 'http://s.geekks.com/social/posttofb.php?llbappkey=1&repostparams='+repostparams, '_system', 'location=no');
...
//var openAuthWindow = cordova.InAppBrowser.open( 'http://s.geekks.com/bblauth.php?llbappkey=1', '_blank', 'location=no,zoom=no,toolbar=no' );
